...
//chatgpt
// Convert base64 back to Blob so frontend can use FileReader
function base64ToFile(base64, mimeType, filename = "ai-image.jpg") {
...
export async function processCarImageWithAI(file) {
    try {
        ...
        try {
            ...
            console.error("AI RAW OUTPUT:", cleanedText);
            return {
                success: false,
                error: "Failed to parse AI response",
            };
...
export async function processCarImageWithAI(file) {
    try {
        ...
        // 🔴 Improved prompt — force clean JSON only!
        const prompt = `
      Analyze the car image and return ONLY a valid JSON object. No explanations, no markdown.
      Include the following fields:
      - make, model, year, color, bodyType, mileage,
      - fuelType, transmission, price, description, confidence
      Example:
      {
        "make": "BMW",
        "model": "M4",
        ...
        "price": "$55000",
        "description": "Well maintained sporty coupe",
        "confidence": 0.9
      }
    `;
        ...
        // 🟨 Attempt to clean unwanted content
        ...
        try {
            ...
            console.error("Raw Cleaned Output:", cleanedText);
        ...
        return {
            success: true,
            data: carDetails,
            imageFile: base64ToFile(base64Image, file.type),
        };
    } catch (error) {
        console.error("❌ processCarImageWithAI Error:", error.message);
    ...
    try {
        ...
        const car = await db.car.create({
            data: {
                ...
                year: carData.year,
                //price: carData.price,
                price: Number(carData.price.toString().replace(/[^0-9.]/g, "")),
                //mileage: carData.mileage,
                mileage: Number(carData.mileage.toString().replace(/[^0-9]/g, "")), 
                color: carData.color,
